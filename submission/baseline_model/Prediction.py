...
from utils.Transf2DDFs import (
    cal_global_allpts,
    cal_global_landmark,
    cal_local_allpts,
    cal_local_landmark,
)
from utils.transform import TransformAccumulation, Transforms
...
class Prediction:
    ...
    def __init__(self, parameters, model_name, data_path_calib, model_path):
        ...
        self.image_points = reference_image_points([480, 640], [480, 640]).to(
            self.device
        )
        ...
        self.pred_dim = self.type_dim(
            self.parameters["PRED_TYPE"],
            self.image_points.shape[1],
            self.data_pairs.shape[0],
        )
        self.label_dim = self.type_dim(
            self.parameters["LABEL_TYPE"],
            self.image_points.shape[1],
            self.data_pairs.shape[0],
        )
    ...
    def generate_prediction_DDF(self, frames, landmark):
        ...
        transformation_global, transformation_local = self.cal_pred_transformations(
            frames
        )
        transformation_global, transformation_local = transformation_global.to(
            self.device
        ), transformation_local.to(self.device)
        # Global displacement vectors for pixel reconstruction
        pred_global_allpts_DDF = cal_global_allpts(
            transformation_global, self.tform_calib_scale, self.image_points
        )
        # Global displacement vectors for landmark reconstruction
        pred_global_landmark_DDF = cal_global_landmark(
            transformation_global, landmark, self.tform_calib_scale
        )
        # Local displacement vectors for pixel reconstruction
        pred_local_allpts_DDF = cal_local_allpts(
            transformation_local, self.tform_calib_scale, self.image_points
        )
        # Local displacement vectors for landmark reconstruction
        pred_local_landmark_DDF = cal_local_landmark(
            transformation_local, landmark, self.tform_calib_scale
        )
    ...
    def cal_pred_transformations(self, frames):
        ...
        while True:
            frames_sub = frames[
                :, idx_f0 : idx_f0 + self.parameters["NUM_SAMPLES"], ...
            ]
            with torch.no_grad():
    ...
    def type_dim(self, label_pred_type, num_points=None, num_pairs=1):
        ...
        return (
            type_dim_dict[label_pred_type] * num_pairs
        )  # num_points=self.image_points.shape[1]), num_pairs=self.pairs.shape[0]
