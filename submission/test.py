...
def main():
    ...
    for scan_name in list(dataset_keys.keys()):
        frames = h5py.File(
            os.path.join(
                data_path_frame,
                scan_name.split("__")[0][3:],
                scan_name.split("__")[1] + ".h5",
            ),
            "r",
        )["frames"][()]
        landmark = h5py.File(
            os.path.join(
                data_path_landmark,
                "landmark_%03d.h5" % int(scan_name.split("__")[0][3:]),
            ),
            "r",
        )[scan_name.split("__")[1]][()]

        # generate predicted DDF
        start = time.time()
        pred_GP, pred_GL, pred_LP, pred_LL = predict_ddfs(
        ...
        labels_GP, labels_GL, labels_LP, labels_LL = generate_GT_ddf.calculate_GT_DDF(
            frames, tforms, landmark
        )
    ...
    metrics.create_dataset(
        "time_elapsed", len(time_elapsed), dtype=time_elapsed.dtype, data=time_elapsed
    )
    metrics.flush()
    metrics.close()
