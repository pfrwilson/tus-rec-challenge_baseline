...
def plot_scans(
    ...
    plot_scan_individual(
        ...
        os.path.join(
            saved_folder,
            "sub%03d__%s"
            % (int(scan_name.split("__")[0][3:]), scan_name.split("__")[1])
            + "_label",
        ),
...
def plot_scan_individual(
    gt, frame, saved_name, step, color, width=4, scatter=8, legend_size=50, legend=None
):
    # plot the scan in 3D
...
def plotting(
    gt, frame, axs, step, color, width=4, scatter=8, legend_size=50, legend=None
):
    # plot surface
    ysize, xsize = frame.shape[-2:]
    grid = np.meshgrid(
        np.linspace(0, 1, ysize), np.linspace(0, 1, xsize), indexing="ij"
    )
    ...
    for i_frame in range(0, gt.shape[0], step):
        ...
        coord[0] = (
            gx[0, 0]
            + (gx[1, 0] - gx[0, 0]) * (grid[0])
            + (gx[0, 1] - gx[0, 0]) * (grid[1])
        )
        coord[1] = (
            gy[0, 0]
            + (gy[1, 0] - gy[0, 0]) * (grid[0])
            + (gy[0, 1] - gy[0, 0]) * (grid[1])
        )
        coord[2] = (
            gz[0, 0]
            + (gz[1, 0] - gz[0, 0]) * (grid[0])
            + (gz[0, 1] - gz[0, 0]) * (grid[1])
        )
        ...
        for i, ax in enumerate(axs):
            ax.plot_surface(
                ...
                coord[1],
                coord[2],
                facecolors=plt.cm.gray(pix_intensities),
                shade=False,
                linewidth=0,
                antialiased=True,
                alpha=0.5,
    ...
    for i, ax in enumerate(axs):
        ...
        ax.plot(
            gt[0, 0, [1, 3]], gt[0, 1, [1, 3]], gt[0, 2, [1, 3]], "b", linewidth=width
        )
        ax.plot(
            gt[0, 0, [3, 2]], gt[0, 1, [3, 2]], gt[0, 2, [3, 2]], "b", linewidth=width
        )
        ax.plot(
            gt[0, 0, [2, 0]], gt[0, 1, [2, 0]], gt[0, 2, [2, 0]], "b", linewidth=width
        )
...
def plot_scan_label_pred(
    gt, pred, frame, color, saved_name, step, width=4, scatter=8, legend_size=50
):
    # plot the scan in 3D
    ...
    plotting(
        gt, frame, axs, step, color[0], width=4, scatter=8, legend_size=50, legend="GT"
    )
    plotting(
        pred,
        frame,
...
def select4pts(GP, tform_calib_scale, image_points, frames):
    ...
    global_allpts = (
        GP
        + torch.matmul(tform_calib_scale, image_points)[0:3, :]
        .expand(GP.shape[0], -1, -1)
        .numpy()
    )
    global_four = global_allpts[
