...
class LabelTransform:
    ...
    def __init__(
        ...
        self.image_points_in_tool = torch.matmul(
            self.tform_image_to_tool, self.image_points
        )
        ...
        if self.label_type == "point":
            ...
            self.call_function = self.to_transform_tool2tool
    ...
    def to_points(self, tforms, tforms_inv=None):
        _tforms = self.to_transform_tool2tool(tforms, tforms_inv)
        return torch.matmul(
            _tforms, torch.matmul(self.tform_image_pixel_to_mm, self.image_points)
        )[
            :, :, 0:3, :
        ]  # [batch,num_pairs,(x,y,z,1),num_image_points]

    def to_transform_tool2tool(self, tforms, tforms_inv):
        ...
        tform_tool1_to_tool0 = torch.matmul(
            tforms_world_to_tool0, tforms_tool1_to_world
        )  # tform_tool1_to_tool0
        # the returned matrix, which is the label, is Orthogonal Matrix
        return torch.matmul(
            self.tform_tool_to_image_mm[None, None, ...],
            torch.matmul(
                tform_tool1_to_tool0, self.tform_image_mm_to_tool[None, None, ...]
            ),
        )  # tform_image1_mm_to_image0_mm

    def to_parameters(self, tforms, tforms_inv):
        _tforms = self.to_transform_tool2tool(tforms, tforms_inv)
        # only Orthogonal Matrix can be converted to Euler angles
        Rotation = pytorch3d.transforms.matrix_to_euler_angles(
            _tforms[:, :, 0:3, 0:3], "ZYX"
        )
...
class PredictionTransform:
    ...
    def transform_to_parameter(self, _tforms):
        ...
        _tforms = torch.cat(
            (_tforms.reshape(-1, self.num_pairs, 3, 4), last_rows), axis=2
        )
        Rotation = pytorch3d.transforms.matrix_to_euler_angles(
            _tforms[:, :, 0:3, 0:3], "ZYX"
        )
        ...
        for i in range(pts.size()[0]):
            for j in range(pts.size()[1]):
                ret_R, ret_t = rigid_transform_3D(
                    torch.matmul(self.tform_image_pixel_to_mm, self.image_points)[
                        0:3, :
                    ],
                    pts[i, j, 0:3, :],
                )
    ...
    def transform_to_transform(self, transform):
        last_rows = torch.cat(
            [
                torch.zeros_like(transform[..., 0])[..., None, None].expand(
                    -1, -1, 1, 3
                ),
        ...
        transform = torch.cat(
            (transform.reshape(-1, self.num_pairs, 3, 4), last_rows), axis=2
        )
    ...
    def quaternion_to_transform(self, quaternion):
        ...
        last_rows = torch.cat(
            (
                torch.zeros_like(tforms[0, 0, :, 0]),
                torch.ones_like(quaternion[0, 0, 0:1]),
            ),
            axis=0,
        ).expand(list(tforms.shape[0:2])[0], list(tforms.shape[0:2])[1], 1, 4)
    ...
    def quaternion_to_parameter(self, quaternion):
        ...
        params = self.transform_to_parameter(
            tforms[:, :, 0:3, :].reshape(tforms.shape[0], self.num_pairs, -1)
        )
        return params

    def quaternion_to_point(self, quaternion):
        tforms = self.quaternion_to_transform(quaternion)
        points = self.transform_to_point(
            tforms[:, :, 0:3, :].reshape(tforms.shape[0], self.num_pairs, -1)
        )
        return points
    ...
    def param_to_transform(self, params):
        ...
        last_rows = torch.cat(
            ...
            axis=0,
        ).expand(list(matrix.shape[0:2])[0], list(matrix.shape[0:2])[1], 1, 4)
        _tforms = torch.cat((transform, last_rows), axis=2)
...
class TransformAccumulation:
    ...
    def __init__(
        ...
        self.tform_tool_to_image = torch.linalg.inv(self.tform_image_to_tool)
        self.tform_tool_to_image_mm = torch.linalg.inv(self.tform_image_mm_to_tool)
        self.tform_image_mm_to_image_pixel = torch.linalg.inv(
            self.tform_image_pixel_to_image_mm
        )

    def __call__(self, tform_1_to_0, tform_2_to_1):

        tform_img1_mm_to_img0_mm = (
            tform_1_to_0  # transformation from image1 to image0 in mm
        )
        tform_img2_mm_to_img1_mm = tform_2_to_1
        # transformation from image2 to image0 in mm
        tform_img2_mm_to_img0_mm = torch.matmul(
            tform_img1_mm_to_img0_mm, tform_img2_mm_to_img1_mm
        )
        # transformation from image2 to image0 in mm
