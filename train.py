...
from utils.transform import LabelTransform, PointTransform, PredictionTransform
...
with open(
    os.getcwd() + "/" + opt.SAVE_PATH + "/" + "data_pairs.json", "w", encoding="utf-8"
) as fp:
    json.dump(data_pairs.cpu().numpy().tolist(), fp, ensure_ascii=False, indent=4)

# all avaliable data
dataset_all = Dataset(
    data_path=opt.DATA_PATH, num_samples=opt.NUM_SAMPLES, sample_range=opt.SAMPLE_RANGE
)
...
for idx, ds in enumerate(dset_folds):
    ds.write_json(
        os.path.join(os.getcwd(), opt.SAVE_PATH, "fold_{:02d}.json".format(idx))
    )
...
val_loader = torch.utils.data.DataLoader(
    dset_val, batch_size=1, shuffle=False, num_workers=8, pin_memory=True
)


## load calibration metric
tform_calib_scale, tform_calib_R_T, tform_calib = read_calib_matrices(
    os.path.join(os.getcwd(), opt.FILENAME_CALIB)
)
    ...
    model.load_state_dict(
        torch.load(
            os.path.join(
                ...
                opt.SAVE_PATH,
                "saved_model",
                "model_epoch" + str(opt.retrain_epoch),
            ),
...
for epoch in range(int(opt.retrain_epoch), int(opt.retrain_epoch) + opt.NUM_EPOCHS):
    ...
    # validation
    if epoch in range(
        int(opt.retrain_epoch), int(opt.retrain_epoch) + opt.NUM_EPOCHS, opt.val_fre
    ):
        ...
        val_loss_min, val_dist_min = save_best_network(
            opt,
            model,
            epoch,
            epoch_loss_val,
