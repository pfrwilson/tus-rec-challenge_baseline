...
with open(
    os.getcwd() + "/" + opt.SAVE_PATH + "/" + "data_pairs.json", "r", encoding="utf-8"
) as f:
...
dset_test = Dataset.read_json(
    os.path.join(os.getcwd(), opt.SAVE_PATH, opt.FILENAME_TEST), num_samples=-1
)

# generate the folder to save the results
saved_folder_test = os.path.join(
    os.getcwd(), opt.SAVE_PATH, "testing", "testing_test_results"
)
...
Evaluation_test = Evaluation(
    opt, device, dset_test, "best_validation_dist_model", data_pairs, saved_folder_test
)
