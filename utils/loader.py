...
class Dataset:
    ...
    def __init__(self, data_path, num_samples, sample_range, indices_in_use=None):
        ...
        self.subs = [
            f
            for f in os.listdir(os.path.join(os.getcwd(), self.data_path))
        ...
        self.scans = [
            f
            for f in os.listdir(os.path.join(os.getcwd(), self.data_path, self.subs[0]))
            if f.endswith(".h5")
        ]
        ...
        if indices_in_use is None:
            ...
            self.indices_in_use = [
                (i_sub, i_scn)
                for i_sub in range(len(self.subs))
                for i_scn in range(len(self.scans))
            ]
        # use selected scans
        elif all([isinstance(t, tuple) for t in indices_in_use]):
            self.indices_in_use = indices_in_use
        elif isinstance(indices_in_use[0], list) and isinstance(
            indices_in_use[1], list
        ):
            self.indices_in_use = [
                (i_sub, i_scn)
                for i_sub in indices_in_use[0]
                for i_scn in indices_in_use[1]
            ]
        else:
            raise (
                "indices_in_use should be a list of tuples (idx_subject, idx_scans) of two lists, [indices_subjects] and [indices_scans]."
            )
        ...
        if num_samples < 2:
            if num_samples == -1:
                ...
                raise (
                    "num_samples should be greater than or equal to 2, or -1 for sampling all frames."
                )
    ...
    def partition_by_ratio(self, ratios, randomise=False):
        ...
        print(
            "Partitioning into %d sets with a normalised ratios %s,"
            % (num_sets, ratios)
        )
        ...
        idx_all = [
            np.where(np.array(fir_ele) == sub_ind[i])[0] for i in range(len(sub_ind))
        ]
        idx_all = [l.tolist() for l in idx_all]
        indices_sets = None
        for n0, n1 in zip(
            [sum(set_sizes[:ii]) for ii in range(num_sets)], set_sizes
        ):  # get the index tuples for all sets
            idx_list = [x for xs in idx_all[n0 : n0 + n1] for x in xs]
            if indices_sets == None:
                indices_sets = [
                    [self.indices_in_use[idx_list_i] for idx_list_i in idx_list]
                ]
            else:
                indices_sets.append(
                    [self.indices_in_use[idx_list_i] for idx_list_i in idx_list]
        ...
        return [
            Dataset(
                ...
                indices_in_use=idx_list,
            )
            for idx_list in indices_sets
        ]
    ...
    def __getitem__(self, idx):
        ...
        scans = [
            f
            for f in os.listdir(os.path.join(self.data_path, self.subs[indices[0]]))
            if f.endswith(".h5")
        ]
        if len(scans) != 24:
        ...
        h5file = h5py.File(
            os.path.join(
                os.getcwd(), self.data_path, self.subs[indices[0]], fn_mha[indices[1]]
            ),
            "r",
        )
    ...
    def frame_sampler(self, n):
        ...
        """
        n0 = random.randint(
            0, n - self.sample_range
        )  # sample the start index for the range
        idx_frames = random.sample(
            range(n0, n0 + self.sample_range), self.num_samples
        )  # sample indices
        idx_frames.sort()
        return idx_frames
    ...
    def read_json(jason_filename, num_samples=None):
        ...
        with open(jason_filename, "r", encoding="utf-8") as f:
            ...
            return Dataset(
                ...
                indices_in_use=[
                    tuple(ids) for ids in obj["indices_in_use"]
                ],  # convert to tuples from json string
            )
