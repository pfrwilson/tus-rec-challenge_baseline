...
class Evaluation:
    ...
    def __init__(self, opt, device, dset, model_name, data_pairs, saved_folder):
        ...
        self.pred_dim = type_dim(
            self.opt.PRED_TYPE, self.image_points.shape[1], self.data_pairs.shape[0]
        )
        self.label_dim = type_dim(
            self.opt.LABEL_TYPE, self.image_points.shape[1], self.data_pairs.shape[0]
        )
        ...
        self.model.load_state_dict(
            torch.load(
                os.path.join(
                    os.getcwd(), self.opt.SAVE_PATH, "saved_model", self.model_name
                ),
    ...
    def calculate_GT_DDF(self, scan_index):
        ...
        frames, tforms = (
            torch.tensor(t)[None, ...].to(self.device) for t in [frames, tforms]
        )
        tforms_inv = torch.linalg.inv(tforms)
        landmark_file = h5py.File(
            os.path.join(
                os.getcwd(), self.opt.LANDMARK_PATH, "landmark_%03d.h5" % indices[0]
            ),
            "r",
        )
        landmark = torch.from_numpy(landmark_file[scan_name][()])

        self.labels_global_allpts_DDF, self.labels_global_four = (
            self.cal_label_global_allpts(frames, tforms, tforms_inv)
        )
        self.labels_global_landmark_DDF = self.cal_label_global_landmark(
            tforms, tforms_inv, landmark
        )
        self.labels_local_allpts_DDF = self.cal_label_local_allpts(tforms, tforms_inv)
        self.labels_local_landmark_DDF = self.cal_label_local_landmark(
            tforms, tforms_inv, landmark
        )

    def cal_label_global_allpts(self, frames, tforms, tforms_inv):
        ...
        labels_global_allpts = torch.squeeze(
            transform_label_global_all(tforms, tforms_inv)
        )
        # DDF in mm, from current frame to frame 0
        labels_global_allpts_DDF = labels_global_allpts - torch.matmul(
            self.tform_calib_scale.to(self.device), self.image_points
        )[0:3, :].expand(labels_global_allpts.shape[0], -1, -1)
        ...
        # [  1.,   1.,   1.,   1.]])
        labels_global_four = (
            labels_global_allpts[
                ...,
                [
                    0,
                    frames.shape[-1] - 1,
                    (frames.shape[-2] - 1) * frames.shape[-1],
                    -1,
                ],
            ]
            .cpu()
        ...
        # add the first frame
        first_frame_coord_all = torch.matmul(
            self.tform_calib_scale, self.image_points.cpu()
        )[0:3, :]
        first_frame_coord = first_frame_coord_all.numpy()[
            ...,
            [0, frames.shape[-1] - 1, (frames.shape[-2] - 1) * frames.shape[-1], -1],
        ][None, ...]
        labels_global_four = np.concatenate(
            (first_frame_coord, labels_global_four), axis=0
        )

        return labels_global_allpts_DDF, labels_global_four

    def cal_label_global_landmark(self, tforms, tforms_inv, landmark):
        ...
        tforms_each_frame2frame0_gt = torch.squeeze(
            transform_label_global_landmark(tforms, tforms_inv)
        )
        ...
        for i in range(len(landmark)):
            ...
            labels_global_landmark[:, i] = (
                torch.matmul(
                    tforms_each_frame2frame0_gt[i],
                    torch.matmul(self.tform_calib_scale.to(self.device), pts_coord),
                )[0:3]
                - torch.matmul(self.tform_calib_scale.to(self.device), pts_coord)[0:3]
            )
    ...
    def cal_label_local_allpts(self, tforms, tforms_inv):
        ...
        labels_local_allpts_DDF = labels_local_allpts - torch.matmul(
            self.tform_calib_scale.to(self.device), self.image_points
        )[0:3, :].expand(labels_local_allpts.shape[0], -1, -1)
    ...
    def cal_label_local_landmark(self, tforms, tforms_inv, landmark):
        # generate required transformations
        data_pairs_local_landmark = torch.tensor(
            [[n0 - 1, n0] for n0 in landmark[:, 0]]
        )
        ...
        tforms_each_frame_to_prev_frame = torch.squeeze(
            transform_label_local_landmark(tforms, tforms_inv)
        )
        ...
        for i in range(len(landmark)):
            ...
            labels_local_landmark[:, i] = (
                torch.matmul(
                    tforms_each_frame_to_prev_frame[i],
                    torch.matmul(self.tform_calib_scale.to(self.device), pts_coord),
                )[0:3]
                - torch.matmul(self.tform_calib_scale.to(self.device), pts_coord)[0:3]
            )
    ...
    def generate_pred_values(self, scan_index):
        ...
        (
            ...
            self.predictions_global_allpts_DDF,
            self.predictions_global_four,
        ) = self.cal_pred_global_allpts(frames)
        self.pred_global_landmark_DDF = self.cal_pred_global_landmark(
            transformation_global, landmark
        )
        self.prediction_local_allpts_DDF = self.cal_pred_local_allpts(
            transformation_local
        )
        self.pred_local_landmark_DDF = self.cal_pred_local_landmark(
            transformation_local, landmark
        )
    ...
    def cal_pred_global_allpts(self, frames):
        ...
        predictions_global_allpts = torch.zeros(
            (frames.shape[1] - 1, 3, self.image_points.shape[-1])
        )
        ...
        interval_pred = (
            torch.squeeze(self.data_pairs[Pair_index])[1]
            - torch.squeeze(self.data_pairs[Pair_index])[0]
        )

        while True:

            frames_sub = frames[:, idx_f0 : idx_f0 + self.opt.NUM_SAMPLES, ...].to(
                self.device
            )
            with torch.no_grad():
                outputs = self.model(frames_sub)
                # transform prediction into transformtion, to be accumulated
                preds_transf = self.transform_to_transform(outputs)[
                    0, Pair_index, ...
                ]  # use the transformtion from image 1 to 0
                transformation_local[idx_f0] = preds_transf
                pts_cord, prev_transf = self.transform_accumulation(
                    prev_transf, preds_transf
                )
                transformation_global[idx_f0] = prev_transf
                predictions_global_allpts[idx_f0] = pts_cord[
                    0:3, ...
                ].cpu()  # global displacement vectors for pixel reconstruction
        ...
        if self.opt.NUM_SAMPLES > 2:
            ...
            transformation_local[idx_f0:, ...] = torch.eye(4).expand(
                transformation_local[idx_f0:, ...].shape[0], -1, -1
            )
            transformation_global[idx_f0:, ...] = transformation_global[
                idx_f0 - 1
            ].expand(transformation_global[idx_f0:, ...].shape[0], -1, -1)
            predictions_global_allpts[idx_f0:, ...] = predictions_global_allpts[
                idx_f0 - 1
            ].expand(predictions_global_allpts[idx_f0:, ...].shape[0], -1, -1)

        predictions_global_allpts_DDF = (
            predictions_global_allpts.numpy()
            - torch.matmul(self.tform_calib_scale.to(self.device), self.image_points)[
                0:3, :
            ]
            .expand(predictions_global_allpts.shape[0], -1, -1)
            .cpu()
            .numpy()
        )

        # store coordinates of four corner points for plot
        predictions_global_four = predictions_global_allpts[
            ...,
            [0, frames.shape[-1] - 1, (frames.shape[-2] - 1) * frames.shape[-1], -1],
        ]
        # add the first frame
        ...
        predictions_global_four = np.concatenate(
            (first_frame_coord, predictions_global_four), axis=0
        )
    ...
    def cal_pred_global_landmark(self, transformation_global, landmark):
        ...
        for i in range(len(landmark)):
            ...
            pred_global_landmark[:, i] = (
                torch.matmul(
                    transformation_global[landmark[i][0] - 1].to(self.device),
                    torch.matmul(self.tform_calib_scale.to(self.device), pts_coord),
                )[0:3]
                - torch.matmul(self.tform_calib_scale.to(self.device), pts_coord)[0:3]
            )
    ...
    def cal_pred_local_allpts(self, transformation_local):
        ...
        prediction_local_allpts = torch.matmul(
            transformation_local.to(self.device),
            torch.matmul(self.tform_calib_scale.to(self.device), self.image_points),
        )
        prediction_local_allpts_DDF = prediction_local_allpts[:, 0:3, :] - torch.matmul(
            self.tform_calib_scale.to(self.device), self.image_points
        )[0:3, :].expand(prediction_local_allpts.shape[0], -1, -1)
    ...
    def cal_pred_local_landmark(self, transformation_local, landmark):
        ...
        for i in range(len(landmark)):
            ...
            pred_local_landmark[:, i] = (
                torch.matmul(
                    transformation_local[landmark[i][0] - 1].to(self.device),
                    torch.matmul(self.tform_calib_scale.to(self.device), pts_coord),
                )[0:3]
                - torch.matmul(self.tform_calib_scale.to(self.device), pts_coord)[0:3]
            )
    ...
    def scan_plot(self, scan_index):
        ...
        plot_scan(
            ...
            frames,
            os.path.join(
                saved_img_path, "sub%03d__%s" % (indices[0], scan_name) + "_label"
            ),
            step=frames.shape[0] - 1,
            color=color[0],
            width=4,
            scatter=8,
            legend_size=50,
        ...
        plot_scan(
            ...
            frames,
            os.path.join(
                saved_img_path, "sub%03d__%s" % (indices[0], scan_name) + "_pred"
            ),
            step=frames.shape[0] - 1,
            color=color[1],
            width=4,
        ...
        plot_scan_label_pred(
            ...
            color,
            os.path.join(
                saved_img_path, "sub%03d__%s" % (indices[0], scan_name) + "_pred_label"
            ),
            step=frames.shape[0] - 1,
            width=4,
            scatter=8,
