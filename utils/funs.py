...
def pair_samples(num_samples, num_pred, single_interval):
    ...
    if single_interval == 0:
        return torch.tensor(
            [
                [n0, n1]
                for n1 in range(num_samples - num_pred, num_samples)
                for n0 in range(n1)
            ]
        )
    else:
        return torch.tensor(
            [
                [n1 - single_interval, n1]
                for n1 in range(single_interval, num_samples, single_interval)
            ]
        )
...
def save_best_network(
    ...
    if running_loss_val < val_loss_min:
        ...
        with open(file_name, "a") as opt_file:
            opt_file.write(
                "------------ best validation loss result - epoch %s: -------------\n"
                % (str(epoch_label))
            )
        if opt.multi_gpu:
            torch.save(
    ...
    if running_dist_val < val_dist_min:
        ...
        with open(file_name, "a") as opt_file:
            opt_file.write(
                "------------ best validation dist result - epoch %s: -------------\n"
                % (str(epoch_label))
            )
...
def write_to_txt(opt, epoch, loss_dists):
    ...
    file_name_train = os.path.join(
        os.getcwd(), opt.SAVE_PATH, "train_results", "train_loss.txt"
    )
    with open(file_name_train, "a") as opt_file_train:
        print(
            "[Epoch %d], train-loss=%.3f, train-dist=%.3f"
            % (epoch, train_epoch_loss, train_epoch_dist),
            file=opt_file_train,
        )

    file_name_val = os.path.join(
        os.getcwd(), opt.SAVE_PATH, "val_results", "val_loss.txt"
    )
    with open(file_name_val, "a") as opt_file_val:
        print(
            "[Epoch %d], val-loss=%.3f, val-dist=%.3f"
            % (epoch, epoch_loss_val, epoch_dist_val),
            file=opt_file_val,
        )
...
def print_info(epoch, loss, dist, opt, train_val):
    ...
    if epoch in range(
        int(opt.retrain_epoch), int(opt.retrain_epoch) + opt.NUM_EPOCHS, opt.FREQ_INFO
    ):
        if train_val == "train":
            print(
                "[Epoch %d] train-loss=%.3f, train-dist=%.3f"
                % (epoch, loss, dist.mean())
            )
        elif train_val == "val":
            print(
                "[Epoch %d] val-loss=%.3f, val-dist=%.3f" % (epoch, loss, dist.mean())
            )
...
def save_model(model, epoch, opt):
    ...
    if epoch in range(
        int(opt.retrain_epoch), int(opt.retrain_epoch) + opt.NUM_EPOCHS, opt.FREQ_SAVE
    ):
        ...
        if opt.multi_gpu:
            torch.save(
                model.module.state_dict(),
                os.path.join(
                    os.getcwd(), opt.SAVE_PATH, "saved_model", "model_epoch%08d" % epoch
                ),
            )
        else:
            torch.save(
        ...
        if len(saved_models) > 4:
            os.remove(
                os.path.join(
                    os.getcwd(), opt.SAVE_PATH, "saved_model", sorted(saved_models)[0]
                )
            )
